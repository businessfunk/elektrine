defmodule ElektrineWeb.EmailLive.Spam do
  ...
  def mount(_params, _session, socket) do
    ...
    if connected?(socket) do
      ...
      Logger.info("Spam LiveView subscribed to PubSub topic: user:#{user.id}")
    end
  ...
  def handle_event("refresh", _params, socket) do
    ...
    pagination = Email.list_spam_messages_paginated(socket.assigns.mailbox.id, page, 20)
    ...
    {:noreply,
     ...
     |> stream(:messages, pagination.messages, reset: true)
     |> put_flash(:info, "Spam folder refreshed")}
  end
  ...
  def handle_event("delete", %{"id" => id}, socket) do
    ...
    if message && message.mailbox_id == socket.assigns.mailbox.id do
      ...
      Logger.info("Deleted spam message #{id}, updating stream")
  ...
  @impl true
  def handle_event("mark_not_spam", %{"id" => id}, socket) do
    message = Email.get_message(id)

    if message && message.mailbox_id == socket.assigns.mailbox.id do
      {:ok, _} = Email.mark_as_not_spam(message)
  ...
  def handle_info({:new_email, _message}, socket) do
    # Refresh the spam folder when new emails arrive
    page = socket.assigns.pagination.page
    pagination = Email.list_spam_messages_paginated(socket.assigns.mailbox.id, page, 20)
    unread_count = Email.unread_count(socket.assigns.mailbox.id)
