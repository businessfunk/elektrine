defmodule Elektrine.Email.Postal do
    ...
    if !message_params[:from] || !message_params[:to] do
      ...
      case Mailer.deliver(email) do
        {:ok, response} ->
          ...
          {:ok, %{
            message_id: response.id || "postal-#{:rand.uniform(1000000)}-#{System.system_time(:millisecond)}",
            status: "sent"
          }}
  ...
  def create_test_email(params) do
    ...
    email_data = %{
      message_id: params[:message_id] || "local-#{:rand.uniform(1000000)}-#{System.system_time(:millisecond)}",
      from: params[:from],
      to: params[:to],
  ...
  defp add_email_content(email, text_body, nil), do: Swoosh.Email.text_body(email, text_body || "")
  defp add_email_content(email, nil, html_body), do: Swoosh.Email.html_body(email, html_body || "")
